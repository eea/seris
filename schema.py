...
RegistrationBoolean = fl.Boolean.with_properties(
    widget="registration_checkbox")
...
countries_mapping = _load_json("refdata/countries.json")
...
class IsInteger(Validator):
    ...
    def validate(self, element, state):
        if element.properties.get("not_empty_error"):
            self.fail = fl.validation.base.N_(
                element.properties["not_empty_error"])
        else:
            self.fail = fl.validation.base.N_(
                u'%(u)s is not a valid value for %(label)s.')
...
class AccessibleCountries(Validator):
    ...
    def validate(self, element, state):
        ...
        if element.value == []:
            ...
            return False
        if not (set(element.value).issubset(set(accessible_countries)) or
                'Administrator' in roles):
            element.errors.append(
                "User %s has access only for: %s" % (user_id, (', ').join(
                    accessible_countries)))
            return False
        ...
        for country in countries:
            ...
            if subregions:
                if not check_common(subregions, element.value):
                    element.errors.append(
                        'At least one sub-region must be '
...
ReportSchema = fl.Dict.with_properties(widget="schema") \
    ...
    CommonDict.named('header')
        ...
        fl.List.named('country')
            ...
            .with_properties(value_labels=countries_mapping)
    ...
    CommonDict.named('details')
        ...
        .using(label=u"Title - in original language", optional=False)
    ...
    CommonDict.named('structure')
        ...
        fl.Dict.named('indicators_usage')
            ...
            IndicatorEnum.named('to_assess_progress')
                         ...
                         .using(
                             label=u"To asses progress to target/threshold"),
            ...
            IndicatorEnum.named('to_evaluate')
                         .valued(*indicators_usage)
                         .using(
                             label=u"To rank/evaluate (e.g. with 'smileys')"),
